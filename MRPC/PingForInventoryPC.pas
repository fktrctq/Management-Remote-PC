...
try
  ...
  begin
   ...
   Log_write(1,'Hardware',HostName+' WSAStartup. '+SysErrorMessage(WSAGetLastError));
   result:=false;
  end;
  ...
  begin
  Log_write(1,'Hardware',HostName+' WSACleanup. '+SysErrorMessage(WSAGetLastError));
  //raise Exception.Create('WSACleanup');
  end;

except on E: Exception do
begin
Log_write(2,'Hardware',HostName+' Общая ошибка GetHostByName. '+e.Message);
result:=false;
end;
end;
end;
...
try
  ...
  if not (Assigned(getaddrinfo) and Assigned (freeaddrinfo)) then
   begin
    ...
    Log_write(2,'Hardware',HostName+' Необходимые процедуры или функции не найдены в библиотеке Ws2_32.dll - '+SysErrorMessage(GetLastError));
    result:=false;
    Exit;
  end;
  try
    ...
    begin
    Log_write(2,'Hardware',HostName+' Ошибка при вызове функции WSAStartup. '+SysErrorMessage(WSAGetLastError));
    result:=false;
    end;
    rez1 := nil;
    rez2 := nil;
...
begin
Log_write(2,'Hardware',HostName+' Общая ошибка Getaddrinfo. '+e.Message);
result:=false;
end;
end;
end;
...
begin
Log_write(2,'Hardware',HostName+' Общая ошибка IdIcmp. '+e.Message);
result:=false;
end;
end;
end;
