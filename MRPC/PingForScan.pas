...
try
  ...
  begin
  Log_write(1,'Scan',HostName+' WSAStartup. '+SysErrorMessage(WSAGetLastError));
  ...
  Begin
    ...
    begin
     //Log_write(1,'Scan',HostName+' Gethostbyname. '+SysErrorMessage(WSAGetLastError));
     result:=false;
    end;
  End;
  if WSACleanup <> 0 then
  Log_write(1,'Scan',HostName+' WSACleanup. '+SysErrorMessage(WSAGetLastError));
 //raise Exception.Create('WSACleanup');

except on E: Exception do
begin
Log_write(2,'Scan',HostName+' Общая ошибка GetHostByName. '+e.Message);
result:=false;
end;
end;
end;
...
try
  ...
  if not (Assigned(getaddrinfo) and Assigned (freeaddrinfo)) then
   begin
    ...
    Log_write(2,'Scan',HostName+' Необходимые процедуры или функции не найдены в библиотеке Ws2_32.dll - '+SysErrorMessage(GetLastError));
    result:=false;
    Exit;
  end;
  try
    ...
    begin
    Log_write(2,'Scan',HostName+' Ошибка при вызове функции WSAStartup. '+SysErrorMessage(WSAGetLastError));
    result:=false;
    end;
    rez1 := nil;
    rez2 := nil;
    ...
    if error = 0 then
     begin
      ...
      if rez2^.ai_family = AF_INET then
       ...
       begin
       //Log_write(1,'Scan',HostName+' Getaddrinfo. '+SysErrorMessage(WSAGetLastError));
       result:=false;
       end;
...
begin
Log_write(2,'Scan',HostName+' Общая ошибка Getaddrinfo. '+e.Message);
result:=false;
end;
end;
end;
...
begin
Log_write(2,'Scan',HostName+' Общая ошибка IdIcmp. '+e.Message);
result:=false;
end;
end;
end;
