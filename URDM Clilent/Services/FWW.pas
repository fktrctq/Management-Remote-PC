...
try
    ...
    try
      ...
      if ReadFW( 'FW','uRDMServer') then
      if not EnumerateFirewallRules('uRDM','uRDMServer') then  // если нет правила для приложения то создаем   // uRDMServer
       begin
       if ReadKey('Server','Port')<>'' then
       AddApplicationRule(strtoint(ReadKey('Server','Port')),'C:\Program Files (x86)\uRDMServer\uRDMServer.exe','uRDM','uRDMServer');
      end;
